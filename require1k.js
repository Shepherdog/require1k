...
R = (function (document, undefined) {
	...
	function getFileExt(fname) {
		...
		if (index !== -1) {
			fname = fname.substring(index + 1); // Keep path without its segments
		}
		index = fname.lastIndexOf(".");
		return index !== -1
			? fname.substring(index + 1) // Only keep file extension
			: ""; // No extension found
	}
	...
	function deepLoad(module, callback, parentLocation, id) {
		...
		request.onload = function (deps, count) {
			if (request.status == 200 || module.t) {
				...
				deps = [];
				(module.t = module.t || request.response).replace(/(\/\*([\s\S]*?)\*\/)|(\/\/(.*)$)/gm, '').replace(/(?:^|[^\w\$_.])require\s*\(\s*["']([^"']*)["']\s*\)/g, function (_, id) {
					deps.push(id);
				});
				count = deps.length;
				function loaded() {
	...
	function resolveModuleOrGetExports(baseOrModule, relative, resolved) {
		...
		if (relative) {
			...
			// in node_modules
			var fileExt = getFileExt(relative);
			if (!!fileExt && fileExt != 'min') {
			...
			baseElement.href = "";
			return (MODULES[resolved] = MODULES[resolved] || {l: resolved, c: contentType});
		}
		...
		if (!baseOrModule[tmp]) {
			if (baseOrModule.c == 'text') {
				return baseOrModule.t;
			} else {
